...
namespace Utilities
{
  ...
  namespace CUDA
  {
    ...
    /**
     * Allocate @p n_elements on the host (pinned memory).
     */
    template <typename T>
    inline void
    host_pinned_malloc(T *&pointer, const unsigned int n_elements)
    {
      cudaError_t cuda_error_code =
        cudaMallocHost(&pointer, n_elements * sizeof(T));
      AssertCuda(cuda_error_code);
    }
    ...
    /**
     * Free memory on the host (pinned memory).
     */
    template <typename T>
    inline void
    host_pinned_free(T *&pointer)
    {
      cudaError_t cuda_error_code = cudaFreeHost(pointer);
      AssertCuda(cuda_error_code);
      pointer = nullptr;
    }
    ...
    /**
     * Allocator to be used for `std::unique_ptr` pointing to host pinned memory.
     */
    template <typename Number>
    Number *
    allocate_host_pinned_data(const std::size_t size)
    {
      Number *host_ptr;
      Utilities::CUDA::host_pinned_malloc(host_ptr, size);
      return host_ptr;
    }
    ...
    /**
     * Deleter to be used for `std::unique_ptr` pointing to host pinned memory.
     */
    template <typename Number>
    void
    delete_host_pinned_data(Number *host_ptr) noexcept
    {
      const cudaError_t error_code = cudaFreeHost(host_ptr);
      AssertNothrowCuda(error_code);
    }
