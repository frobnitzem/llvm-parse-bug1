...
#    endif
    ...
    Partitioner::export_to_ghosted_array_start(
      ...
      const ArrayView<const Number, MemorySpaceType> &locally_owned_array,
      const ArrayView<Number, MemorySpaceType> &      temporary_storage,
      const ArrayView<Number, MemorySpace::Host> &    temporary_storage_host,
      const ArrayView<Number, MemorySpaceType> &      ghost_array,
      const ArrayView<Number, MemorySpace::Host> &      ghost_array_host,
      std::vector<MPI_Request> &                      requests) const
    {
      AssertDimension(temporary_storage.size(), n_import_indices());
      AssertIndexRange(communication_channel, 200);
      ...
      const bool use_larger_set =
        (n_ghost_indices_in_larger_set > n_ghost_indices() &&
         ghost_array.size() == n_ghost_indices_in_larger_set);
      Number *ghost_array_ptr =
        use_larger_set ? ghost_array_host.data() + n_ghost_indices_in_larger_set -
                           n_ghost_indices() :
                         ghost_array_host.data();
      ...
      Number *temp_array_ptr = temporary_storage.data();
      Number *temp_array_host_ptr= temporary_storage_host.data();      
#    if defined(DEAL_II_COMPILER_CUDA_AWARE) && \
      defined(DEAL_II_MPI_WITH_CUDA_SUPPORT)
      // When using CUDAs-aware MPI, the set of local indices that are ghosts
      // indices on other processors is expanded in arrays. This is for
      ...
      if (std::is_same<MemorySpaceType, MemorySpace::CUDA>::value)
        {
          ...
          {
            const int n_blocks =
              1 + (import_indices_flattened_dev[0].second - 1) /
                    (::dealii::CUDAWrappers::chunk_size *
                     ::dealii::CUDAWrappers::block_size);
            ::dealii::LinearAlgebra::CUDAWrappers::kernel::
              gather<<<n_blocks, ::dealii::CUDAWrappers::block_size>>>(
                temp_array_ptr,
                import_indices_flattened_dev[0].first.get(),
                locally_owned_array.data(),
                import_indices_flattened_dev[0].second);
          }
...
#    if defined(DEAL_II_COMPILER_CUDA_AWARE) && \
      defined(DEAL_II_MPI_WITH_CUDA_SUPPORT)
      if (temporary_storage.size()>0)
      {
        cudaError_t cuda_error =
    cudaMemcpy(temporary_storage_host.data(),
         temporary_storage.data(),
         temporary_storage.size() * sizeof(Number),
      ...
      for (unsigned int i = 0; i < n_import_targets; i++)
        {
          ...
          const int ierr =
            MPI_Isend(temp_array_host_ptr,
                      ...
                      &requests[n_ghost_targets + i]);
          AssertThrowMPI(ierr);
          temp_array_host_ptr += import_targets_data[i].second;
        }
    }
...
#    endif
      ...
      AssertIndexRange(n_ghost_indices(), n_ghost_indices_in_larger_set + 1);
      Number *ghost_array_ptr = ghost_array.data();
      Number *ghost_array_host_ptr = ghost_array_host.data();
...
#    if defined(DEAL_II_COMPILER_CUDA_AWARE)
      ...
      {
        cudaError_t cuda_error =
          cudaMemcpy(ghost_array_host.data(),
                     ghost_array.data(),
                     ghost_array.size() * sizeof(Number),
                     cudaMemcpyDeviceToHost);
        AssertCuda(cuda_error);
...
#    if defined(DEAL_II_COMPILER_CUDA_AWARE)
                          ...
                          AssertCuda(cuda_error);
                          */
                          std::copy(ghost_array_host.data() + my_ghosts->first,
                                    ghost_array_host.data() + my_ghosts->second,
                                    ghost_array_host_ptr + offset);
                          std::fill(
                            std::max(ghost_array_host.data() + my_ghosts->first,
                                     ghost_array_host_ptr + offset + chunk_size),
                            ghost_array_host.data() + my_ghosts->second,
                            Number{});                          
#    else
                          Assert(
...
#    endif
          const int ierr =
            MPI_Isend(ghost_array_host_ptr,
                      ghost_targets_data[i].second * sizeof(Number),
                      MPI_BYTE,
                      ghost_targets_data[i].first,
                      mpi_tag,
          ...
          ghost_array_ptr += ghost_targets_data[i].second;
          ghost_array_host_ptr += ghost_targets_data[i].second;          
        }
    }
...
#    endif
    ...
    Partitioner::import_from_ghosted_array_finish(
      const VectorOperation::values                   vector_operation,
      const ArrayView<Number, MemorySpaceType> &temporary_storage,
      const ArrayView<const Number, MemorySpace::Host> &temporary_storage_host,      
      const ArrayView<Number, MemorySpaceType> &      locally_owned_array,
      const ArrayView<Number, MemorySpaceType> &      ghost_array,
      const ArrayView<Number, MemorySpace::Host> &      ghost_array_host,
      std::vector<MPI_Request> &                      requests) const
...
#    else
	  cudaError_t cuda_error =
	    cudaMemcpy(temporary_storage.data(),
		       temporary_storage_host.data(),
		       temporary_storage.size() * sizeof(Number),
		       cudaMemcpyHostToDevice);
	  AssertCuda(cuda_error);
              ...
              const int n_blocks =
                1 + (import_indices_flattened_dev[0].second - 1) /
                      (::dealii::CUDAWrappers::chunk_size *
                       ::dealii::CUDAWrappers::block_size);
              ::dealii::LinearAlgebra::CUDAWrappers::kernel::
                assemble<<<n_blocks, ::dealii::CUDAWrappers::block_size>>>(
                  locally_owned_array.data(),
                  import_indices_flattened_dev[0].first.get(),
                  read_position,
                  import_indices_flattened_dev[0].second);
