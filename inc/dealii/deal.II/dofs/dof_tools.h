...
namespace DoFTools
{
  ...
  template <int dim, int spacedim, typename number>
  void
  make_hanging_node_constraints_from_serial(const DoFHandler<dim, spacedim> &dof_handler,
	                                          const DoFHandler<dim, spacedim> &     dof_handler_par,
	                                          const std::map<dealii::CellId,typename DoFHandler<dim, spacedim>::active_cell_iterator> & cellIdToCellIterMapSer,  
                                            AffineConstraints<number> &      constraints);  
