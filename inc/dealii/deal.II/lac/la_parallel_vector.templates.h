...
#  endif
                                            ...
                                            const bool omit_zeroing_entries)
    {
      ...
      import_data.values_host_pinned.reset();

      // delete previous content in ghost data
      ghost_data.values.reset();
      ghost_data.values_dev.reset();
      ghost_data.values_host_pinned.reset();      
      ...
      if (std::is_same<MemorySpaceType, dealii::MemorySpace::CUDA>::value)
        {
          ...
          */
          ...
          if (ghost_data.values_host_pinned == nullptr)
          {
              if (partitioner->n_ghost_indices() > 0)
                ghost_data.values_host_pinned.reset(
                  Utilities::CUDA::allocate_host_pinned_data<Number>(
                     partitioner->n_ghost_indices()));
          }


          if (import_data.values_host_pinned == nullptr)
          {
              if (partitioner->n_import_indices() > 0)
                import_data.values_host_pinned.reset(
                  Utilities::CUDA::allocate_host_pinned_data<Number>(
          ...
          partitioner->import_from_ghosted_array_start(
            ...
            ArrayView<Number, MemorySpace::CUDA>(data.values_dev.get() +
                                              ...
                                              partitioner->n_ghost_indices()),
            ArrayView<Number, MemorySpace::Host>(ghost_data.values_host_pinned.get(),
                                              partitioner->n_ghost_indices()),
            ArrayView<Number, MemorySpace::CUDA>(import_data.values_dev.get(),
                                              partitioner->n_import_indices()),
            ArrayView<Number, MemorySpace::Host>(import_data.values_host_pinned.get(),
                                              partitioner->n_import_indices()),
            compress_requests);
      ...
      if (std::is_same<MemorySpaceType, MemorySpace::CUDA>::value)
        {
          ...
          */
          ...
          partitioner->import_from_ghosted_array_finish<Number, MemorySpace::CUDA>(
           ...
           import_data.values_host_pinned.get(), partitioner->n_import_indices()),
           ArrayView<Number, MemorySpace::CUDA>(data.values_dev.get(),
                                              partitioner->local_size()),
           ArrayView<Number, MemorySpace::CUDA>(data.values_dev.get() +
                                                partitioner->local_size(),
          ...
          */
          ...
          partitioner->export_to_ghosted_array_finish(
             ArrayView<Number, MemorySpace::CUDA>(
               ...
               partitioner->n_ghost_indices()),
             ArrayView<Number, MemorySpace::Host>(
               ghost_data.values_host_pinned.get(),
               partitioner->n_ghost_indices()),
             update_ghost_values_requests);          
#  endif
        }
...
#  endif
      ...
      std::swap(ghost_data, v.ghost_data);
